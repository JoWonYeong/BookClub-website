let 책 = [
  {
    id: 0,
    url: './images/book0.png',
    title: '46번째 밀실',
    content:
      '눈 덮인 고원 별장 성화장. 매년 크리스마스 때가 되면 자신의별장으로 사람들을 초대하는 밀실 추리소설계의 거장 마카베 세이치.그는 올해 역시 크리스마스 파티를 위해 모인 사람들 앞에서 계속 같은 이야기만 쓰는 데 질렸다며 46번째 밀실 작품을 마지막으로 밀실을 졸업하겠다고 선언하고 자리를 뜬다. 그리고 크리스마스 아침. 밀실트릭의 대가는 밀실 상태의 지하 서고 벽난로에 상반신을 들이박고 죽은 처참한 모습으로 발견되는데… 추리소설가 아리스가와 아리스와 임상범죄학자 히무라 히데오가 괴사건의 수수께끼에 도전한다.',
  },
  {
    id: 1,
  ...
  },
  {
    ...
    title: '옛날에 내가 죽은 집',
    content:
      '7년 전 헤어진 여자친구의 잃어버린 기억을 되찾기 위해 수수께끼 집을 방문, 사건을 추리해나가는 이야기를 담고 있다.',
  },
  {
    id: 3,
  ...
  {
    id: 7,
    url: './images/book7.png',
    title: '용서받지 못한 밤',
    content:
      '내 딸이 아내를 죽였다. 엄마를 죽게 한 네살배기 딸, 아빠는 그 죄를 덮었다. 15년 뒤, 비밀을 아는 누군가로부터 전화가 걸려오는데…',
  },
  {
    id: 8,
    url: './images/book8.png',
    title: '사과는 떨어지지 않는다',
    content:
      '‘엄마의 실종’을 둘러싸고 벌어지는 이야기를 다루고 있는 이번 소설은 주변에 있을 법한 현실적인 캐릭터와 제한된 시간, 현재와 과거를 오가며 맞춰지는 퍼즐식 구성, 각기 다른 진술과 의심으로 증폭되는 쫄깃한 긴장감, 적절하게 깔리는 단서와 마지막 반전까지…',
  },
  {
    id: 9,
    url: './images/book9.png',
    title: '조인계획',
    content:
  ...
  {
    id: 10,
    url: './images/book10.png',
    title: '더러운 손을 거기에 닦지 마',
    content:
  ...
  {
    id: 12,
    url: './images/book12.png',
    title: '안젤리크',
    content:
      '파리 오페라 발레단의 전직 에투알 무용수인 스텔라 페트렌코가 6층 자택에서 추락해 사망한다. 수사 결과 집 안으로 들어가는 유일한 출구인 현관문이 굳게 잠겨 있다.',
  },
];
...
책.forEach((a, i) => {
  // img 요소 만듦 (src에 url, alt에 책제목)
  let 책_img = document.createElement('img');
  책_img.setAttribute('src', a.url);
  ...
  // span.content 요소 만듦(innerHTML = content)
  let 책_content = document.createElement('span');
  책_content.classList.add('content');
  책_content.innerHTML = a.content;

  // input.checked 요소 만듦(type='checkbox' name='book' value='book'+id)
  let 책_checked = document.createElement('input');
  책_checked.classList.add('checked');
  책_checked.setAttribute('type', 'checkbox');
  책_checked.setAttribute('name', 'book');
  책_checked.setAttribute('value', 'book' + i);

  // article.rent-book에 다 붙이기
  let article = document.createElement('article');
  article.classList.add('rent-book');
  article.appendChild(책_img);
  article.appendChild(책_title);
  article.appendChild(책_content);
  article.appendChild(책_checked);

  rent.appendChild(article);
});
